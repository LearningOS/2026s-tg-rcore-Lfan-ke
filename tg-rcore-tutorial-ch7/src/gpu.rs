//! VirtIO-GPU 帧缓冲驱动（第七章图形游戏基建）。
//!
//! 移植自第四章 `gpu.rs`，两点适配第七章环境：
//! 1. **MMIO 槽位**：第七章有块设备占用 `virtio-mmio-bus.0`（`0x1000_1000`），
//!    GPU 落在 `bus.1`（`0x1000_2000`），键盘见 `input.rs` 落在 `bus.2`。
//! 2. **HAL 地址翻译**：第七章有全局 `KERNEL_SPACE`，`virt_to_phys` 复用
//!    `virtio_block.rs` 的做法——经 `KERNEL_SPACE.translate` 查页表，而非
//!    第四章的恒等。GPU 与键盘（`input.rs`）共享本 HAL。
//!
//! 帧缓冲像素格式 `BGRA8888`（内存序 `[B, G, R, A]`），分辨率由设备上报。
//! `setup_framebuffer` 返回的切片即帧缓冲 DMA 区，其物理地址会被按需映射进
//! “裁判/渲染”进程的用户空间 `0x4000_0000`（见 `Process::map_framebuffer`）。
